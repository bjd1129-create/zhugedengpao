...
⚠️ OpenClaw 安全警示（2026-04-01 更新）：
- 255+ GitHub安全公告，156个CVE追踪中
- 2026年2月：首次确认 in-the-wild infostealer攻击
- ClawHub市场发现341个恶意技能
- 21,000+ 公开可访问实例，1.5M tokens泄露
- **防护：** 升级到 v2026.3.31（当前运行版本，Plugin SDK已稳定化），不装来源不明技能，定期轮换API Key

⚠️ **自我进化场景的特殊风险（Lethal Trifecta）：**
- 自我修改型Agent + 私有数据访问 + 外部通信能力 = 极高风险组合
- 来源不明的进化插件可能含恶意代码（优先使用经过社区大量验证的插件）
- 推荐插件均来自主流渠道（Self-Evolve Plugin / Capability Evolver）
...
️ OpenClaw 进化机制（2026-04-02 深度研究更新）：

**自我进化三件套（已验证可实施）：**

1. **Self-Evolve Plugin**（推荐优先部署）
   - GitHub: github.com/longmans/self-evolve | 安装：`git clone ... && openclaw plugins install ./self-evolve`
   - 机制：Q值强化学习 + RAG情景记忆检索，在真实会话中提取「意图-经验-结果」triplets
   - 共享网络：self-evolve.club，可与其他OpenClaw节点共享经验（默认开启，隐私双重脱敏）
   - **三种模式：** balanced（默认，最省token）/ tools_only（仅工具调用时学）/ all（全学习，高消耗）
   - **关键参数：** minAbsReward=0.15, minRewardConfidence=0.55, retrieval.tau=0.85
   - 使用技巧：Praise要明确（"做得好" > "ok"），Criticism要具体
   - 推荐 OpenClaw 2026.3.2+，当前运行版 v2026.3.31

2. **Capability Evolver**（35K+下载量，ClawHub排名第一）
   - 安装：`claw install capability-evolver`
   - 机制：GEP（Guided Evolution Protocol）标准化改进流程，防失控
   - 运行 `/evolve` 扫描 memory/日志找优化机会；任务连续失败3次自动检测并创建workaround
...
**安装顺序建议：** Self-Evolve Plugin → Capability Evolver → Self-Improving Agent
...
**双层记忆系统：**
- 长期记忆（MEMORY.md）：架构决策、踩坑记录
- 日记（memory/YYYY-MM-DD.md）：每日操作、数据、策略
...
**OpenClaw稳定运行7条黄金法则（2026-04-02调研补充）：**
1. 模型路由：不同任务用不同模型，Heartbeat→Haiku，推理→Opus，可省70-80%费用
2. 花费上限：每个API提供商设硬上限，maxIterations=10-15防Runaway Loop
3. 结构化SOUL.md：必须包含Error state behavior、Conversation boundaries、Topic restrictions
4. 配置文件版本控制：防止更新后崩溃无法回滚
5. 技能安全审查：安装前完整阅读SKILL.md代码
6. 监控日志：Self-Healing心跳每小时+每日全量扫描
7. 定期进化复盘：凌晨4点自动复盘+Self-Evolve持续学习

⚠️ 安全警示（2026-04-02 新增）：
- **ZeroClaw 下架事件**：GitHub 组织全网 404，官网仍正常运行，原因不明。
  教训：不要单一依赖来源，定期备份重要技能。
- **ClawHub 安全**：已发现 341 个恶意技能，安装前必须完整阅读 SKILL.md 代码。

⚠️ **Lethal Trifecta 高危场景（2026-04-03 新增）**：
- 自我修改型Agent + 私有数据访问 + 外部通信 = 极高风险组合
- 来源不明的进化插件可能含恶意代码，优先使用社区大量验证的插件
- 早晨研究简报等外部搜索场景：SOUL.md 必须明确禁止 Agent 提出"扩大自身权限"的改进建议

️ AutoSkill 框架（2026-04-02 新增研究）：
- 来源：华东师范大学 + 上海人工智能实验室 | GitHub: ECNU-ICALK/AutoSkill
- 核心思想：技能不是设计出来的，是从真实交互中"长出来"的。
- 双环机制：右环（技能进化：提取经验→显式技能）/ 左环（技能增强：检索→注入→响应）
- 与 Self-Evolve 区别：Self-Evolve 学"经验"，AutoSkill 学"技能结构"，两者互补。
- 技能版本管理：新经验合并到已有技能（非新建），版本号自动递增。
- 案例：professional_text_rewrite 已迭代至 v0.1.34（通过30+次用户反馈优化）。

️ iClaw iOS 应用（2026-04-02 新动态）：
- 已上架 TestFlight，支持多 Agent 架构、JavaScript 沙盒、网页自动化。
- 意义：OpenClaw 生态向移动端扩展，潜在可集成到团队工作流。

️ 记忆系统防崩溃铁律（VelvetShark Memory Masterclass 核心发现）：

OpenClaw 记忆有四层，失败有三种模式：
...
**文件优先原则（最高优先级）：** 所有规则、偏好、指令必须写入 MEMORY.md/AGENTS.md，对话中说的在压缩后可能消失。

**Memory Flush 验证命令：**
```bash
openclaw memory status        # 检查索引状态
openclaw memory search "query"  # 命令行搜索测试
openclaw memory index --force   # 重建索引
```

进化 ≠ 失控：Capability Evolver 的 GEP 流程防失控，Self-Evolve 的 Q值更新是算法驱动而非随意。

️ 记忆三层必须操作（防崩溃铁律，VelvetShark 2026-04 新增）：
- **持久化**：MEMORY.md 和 AGENTS.md 生存 compaction，聊天中的指令不生存，必须写入文件
- **检查 flush**：确认 memory flush 已启用（OpenClaw 内置安全网，默认开启但多数人未检查）
- **强制检索**：AGENTS.md 必须有"行动前必须 search memory"规则，防止 Agent 盲目猜测

️ 早晨研究简报 Cron 模式（OpenClaw Pulse 2026-04 实操模板）：
- **触发**：工作日 8:30 AM（约 £0.02-0.08/次）
- **来源**：Anthropic Engineering Blog + Simon Willison + HN(AI/OpenClaw, 50+points) + GitHub Trending
- **流程**：搜索 → 读取自身 AGENTS.md/TOOLS.md/LESSONS.md → 识别差距 → JSON实验追踪 → 可选 Telegram 简报
- **安全原则**：Agent 只提建议不自动修改文件（防止 prompt injection）；禁止提出扩大自身权限的改进
- **配置**：research-config.json（~/.openclaw/workspace/）+ improvements.json（实验追踪）
