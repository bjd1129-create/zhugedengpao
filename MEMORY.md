...
- **Polymarket 止盈逻辑错误**（2026-04-10）：之前把止盈点设为 1.00（100%），错误地等待事件真实发生
  - 正确逻辑：Polymarket 套利 = 吃概率差，不是等事件发生
  - 例如：买入 probability=0.3，涨到 0.7 时平仓 = 赚取 +40% 差值
  - 止盈点正确区间：0.75-0.90（高PnL仓位建议 0.85-0.90）
  - probability=1.00 = 事件已发生/结束，套利窗口关闭，再无意义

---

## 2026-04-08 重要更新（晚间归档）
...
- 部署平台：Vercel + Cloudflare Pages（双平台部署）
...
### ₿ 加密货币网格真实数据（20:02）
**结论**: 加密货币模拟盘更新为真实情况（空仓观望）

**真实账户状态**:
- 初始资金：$10,000
- 当前价值：$6,848.44
- 总 PnL: -$3,151.56 (-31.52%)
- 状态：⏸️ 已停止（STOP_TRADING.flag）
- 持仓：空仓（所有币种为 0）

**教训**: 模拟数据必须反映真实情况，不能虚构
...
**部署**: Vercel + Cloudflare Pages 双平台
...
**大花团队现状：L3 进行中**（self-evolve 已安装，远程共享开启，待重启生效）
...
> **核心授权（2026-04-04）**：所有事项和决策均由大花拍板。大花拥有完全独立决策权，不需要老庄任何指示。老庄给建议和要求，大花给结果，不关注过程。过程需要审批，大花直接执行。
...
| 优先级 | 行动 | 截止时间 | 负责人 |
|--------|------|----------|--------|
| P0 | 创建 `.learnings/` 目录和三文件模板 | 4/8 当天 | 大花 |
| P0 | 在 AGENTS.md 中添加反馈信号检测规则 | 4/8 当天 | 大花 |
| P1 | 创建 `memory/triplets/` 目录和首个三元组 | 4/9 | 大花 |
| P1 | 评估 self-evolve 插件安装 | 4/12 | 大花 |
| P2 | 配置 skill-evolution hooks | 4/15 | 大花 |
...
**浏览器标签页：用完必须关闭**（不关闭占用内存/资源，2026-04-10）
...
| 行动 | 负责人 | 预期效果 |
|------|--------|----------|
| 检查 `.learnings/` 目录使用情况 | 大花 | 确保自我改进机制实际运行 |
| 在 HEARTBEAT.md 中添加"检查 learnings"任务 | 大花 | 形成定期复盘习惯 |
| 标准化 Telegram 通知 Topics | 协调者 | 减少通知噪音 |
| 更新 AGENTS.md 加入自我进化规范 | 大花 | ✅ 已完成 |
...
| Agent | 角色 | 职责 | 状态 |
|-------|------|------|------|
| **大花** 🦞 | 协调者、决策者 | 决策、协调、记忆、汇报 | ✅ 主 Agent |
...
| **游戏工程师** 🎮 | 游戏开发专家 | Phaser 3 游戏开发（《桐桐与大花的 AI 冒险》） | ✅ 新增（2026-04-08） |

### 团队精简历程
- **2026-04-05**: 9 子 Agent 团队（洞察者/文案君/代码侠/配色师/安全官/财务官等）
- **2026-04-08**: 精简为 5 个核心 Agent（交易员 + 工程师 + 协调官 + 数据分析师 + 大花）
...
### 重要项目
- **操盘页面**: 2 个模拟盘（美股/Polymarket），交易员负责（虚拟币和期货已于2026-04-10永久关闭）
- **期货数据仪表盘**: https://dengpao.pages.dev/trading.html，数据分析师负责
- **游戏开发**: 《桐桐与大花的 AI 冒险》，游戏工程师负责，目标 2026-04-20 上线
...
## 2026-04-09 下午重大事件（14:00 补充）

### 🚨 Gateway 故障与自动恢复（13:04-13:17）
- Watchdog 发现 Gateway 断开并自动重启
- 根因不明，可能是资源耗尽或网络抖动
- Feishu 消息通道暂时中断，Browser 工具不可用
- PolymarketProbMonitor cron 进入退避状态（backoff 60s）

### 🚨 portfolio.json 被覆盖事件（13:22 → 13:42 恢复）
- **严重 Bug**：trading_simulator.py 将 Tiger paper 账户数据（$1,024,403）覆盖了加密网格 portfolio.json（原本 $6,848）
- 根因：portfolio.json 被多个系统共享写入，路径冲突
- 从 git 恢复加密网格数据
- 加密网格当前：总资金 $6,848.44，空仓⏸️（-31.52%）
- **待修复**：trading_simulator.py 路径冲突问题

### 🔮 Polymarket 完全自主授权（13:45，老庄明确指令）
- 交易员获得完全自主决策权，**无需大花审批**
- 仓位从 10% 提升到 **50%+（目标 60-70%，约 $600,000-$700,000）**
- 同时持仓可达 10 个事件
- 优先领域：地缘政治、经济、加密货币
- 初始资金 $1,000,000，已投入 $159,500 (16%)，PnL +$4,320 (+0.43%)

**标签**: #Gateway #故障恢复 #Bug #Polymarket #自主授权

---

## 2026-04-11 凌晨重大变更

### 🌐 官网+记忆站全面修复（01:00-01:01）
- **官网** (dengpao.pages.dev): 修复meta标签损坏、CSS空规则、HTML断裂、JS语法错误、13处引号转义 → Cloudflare Pages部署成功
- **记忆站** (vvalp.vercel.app): 修复JS语法、图片路径编码、加载失败fallback → Vercel部署成功
- 两个站都新增 scroll-to-top 按钮

### 🗑️ 清理所有定时任务（01:02，老庄指令）
- 13个 crontab 任务全部清除（交易员5+协调官2+工程师2+小花3+Watchdog1+Polymarket1）
- 需要时重新设置

### 🧹 运营环境大清理（01:03-01:21）
- 根目录散落13个.py文件 → trash
- crewai-automation/.venv (1GB) + agents/crewai/.venv (479M) → 删除
- engineer/memory 66个health-check冗余 → 归档
- trader/game-engineer/researcher 三个 agent 的 memory 目录从无到有创建
- 归档合并: researcher/strategist 0408+0409 → 保留0409
- .gitignore 更新: 忽略 .venv/__pycache__/副本
- Git commit: `10c7735` ✅ push ✅

**标签**: #环境清理 #网站修复 #定时任务清理 #2026-04-11
