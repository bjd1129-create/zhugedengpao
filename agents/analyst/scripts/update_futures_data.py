...
OUTPUT_DIR = Path("/Users/bjd/Desktop/ZhugeDengpao-Team/agents/analyst/data")
OUTPUT_DIR.mkdir(exist_ok=True)

# 从东方财富抓取的真实数据 (2026-04-08 收盘数据)
# 数据来源：http://quote.eastmoney.com/center/gridlist.html#futures_cffex-_IF
futures_data = [
    {
        'code': 'IFM0',
        'name': '沪深 300 当月连续',
...
def main():
    ...
    # 计算升贴水
    futures_data_processed = calculate_basis(futures_data.copy())
    
    # 检测异常波动
    anomalies = detect_anomalies(futures_data_processed, threshold_pct=3.0)
    
    # 构建输出数据
    output = {
        "timestamp": datetime.now().isoformat(),
        "source": "东方财富",
        "futures": futures_data_processed,
        "anomalies": anomalies,
        "summary": {
            "IF_monthly": next((x["price"] for x in futures_data_processed if "IFM0" in x["code"]), None),
            "IC_monthly": next((x["price"] for x in futures_data_processed if "ICM0" in x["code"]), None),
            "IM_monthly": next((x["price"] for x in futures_data_processed if "IMM0" in x["code"]), None),
            "IH_monthly": next((x["price"] for x in futures_data_processed if "IHM0" in x["code"]), None),
            "max_change_pct": max((abs(x["change_pct"]) for x in futures_data_processed), default=0),
    ...
    for fut in futures_data_processed:
