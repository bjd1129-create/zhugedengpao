#!/usr/bin/env python3
"""
Bybit Grid Trading Simulator v1.0
策略: 密集网格v4.1 (20格×0.5%)
"""
...
INITIAL = 10_000.0
PATH = os.path.expanduser('~/Desktop/ZhugeDengpao-Team/data/trading/bybit_portfolio.json')
STOP_LOSS = 0.95
TAKE_PROFIT = 1.05

SYMBOLS = {
    'BTC': {'bybit': 'BTCUSDT', 'per_grid': 250, 'spacing': 0.005, 'levels': 20},
    'ETH': {'bybit': 'ETHUSDT', 'per_grid': 150, 'spacing': 0.005, 'levels': 20},
    'AVAX': {'bybit': 'AVAXUSDT', 'per_grid': 200, 'spacing': 0.005, 'levels': 20},
    'ADA': {'bybit': 'ADAUSDT', 'per_grid': 200, 'spacing': 0.005, 'levels': 20},
}
...
def get_price(sym):
    try:
        r = requests.get('https://api-testnet.bybit.com/v5/market/tickers',
            params={'category': 'spot', 'symbol': sym}, proxies={'https': P} if P else {}, timeout=10)
        d = r.json().get('list', [{}])[0]
...
def main():
    ...
    prices = {k: get_price(v['bybit']) for k, v in SYMBOLS.items()}
