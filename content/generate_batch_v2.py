...
Sequential generation (API is not thread-safe for auth), parallel downloads
"""
...
def generate_one(idx_num, prompt, aspect_ratio):
    """Generate a single image using direct requests (sequential to avoid auth issues)"""
    ...
    try:
        ...
        if data.get("base_resp", {}).get("status_code") != 0:
            return {
                "number": idx_num, "prompt": prompt, "aspect_ratio": aspect_ratio,
                "output_path": output_path, "success": False,
                "image_url": "", "error": data.get("base_resp", {}).get("status_msg", "Unknown error")
            }
        ...
        try:
            ...
            print(f"  ✅ #{idx_num}: {prompt[:40]}... → saved")
        except Exception as e:
            print(f"  ✅ #{idx_num}: {prompt[:40]}... → URL: {image_url[:60]}... (download failed: {e})")
            return {
                "number": idx_num, "prompt": prompt, "aspect_ratio": aspect_ratio,
                "output_path": output_path, "success": True,  # Still success if we got URL
                "image_url": image_url, "error": f"Download failed: {e}"
            }
        
        return {
            "number": idx_num, "prompt": prompt, "aspect_ratio": aspect_ratio,
            "output_path": output_path, "success": True,
            "image_url": image_url, "error": ""
        }
        ...
        return {
            "number": idx_num, "prompt": prompt, "aspect_ratio": aspect_ratio,
            "output_path": output_path, "success": False,
            "image_url": "", "error": f"Request failed: {e}"
        }
    except Exception as e:
        return {
            "number": idx_num, "prompt": prompt, "aspect_ratio": aspect_ratio,
            "output_path": output_path, "success": False,
            "image_url": "", "error": f"Error: {e}"
        }
...
def main():
    print(f"🚀 开始生成120张图片（顺序执行，避免并发认证问题）...")
    ...
    for i in range(0, len(IMAGES), BATCH_SIZE):
        ...
        print(f"\n{'='*60}")
        print(f"Batch {batch_num}/{total_batches}: 生成第 {batch_items[0][0]}-{batch_items[-1][0]} 张")
        print(f"{'='*60}")
        
        batch_results = []
        for idx, prompt, ratio in batch_items:
            result = generate_one(idx, prompt, ratio)
            batch_results.append(result)
            all_results.append(result)
            # Small delay between requests to be nice to API
            time.sleep(1)
        
        # Write record after each batch
        write_record(all_results)
        print(f"    Batch {batch_num} 完成，已记录")
