...
def gen(prompt, idx, scene_id, variant_idx):
    for attempt in range(5):
        try:
            print(f"  Attempt {attempt+1}/5 for image {idx}...", flush=True)
            r = requests.post(
            ...
            print(f"  API response: {r.status_code}", flush=True)
            if r.status_code == 200:
                ...
                if img_list:
                    ...
                    print(f"[{idx}/120] SUCCESS: {fname}", flush=True)
                    return True
                else:
                    print(f"  No image data in response", flush=True)
            else:
...
def main():
    ...
    print(f"Found {len(existing)} existing images, starting from {start_idx}", flush=True)
    ...
    for idx_offset in range(120 - start_idx + 1):
        ...
        print(f"\n=== Generating image {idx}/120 ===", flush=True)
        ok = gen(prompt, idx, scene_id, var_idx)
        if not ok:
            print(f"FAILED image {idx} after 5 attempts, continuing...", flush=True)
